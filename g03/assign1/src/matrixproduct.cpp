...
void OnMultBlock(int m_ar, int m_br, int bkSize)
{
    ...
    Time1 = clock();
     for(i = 0; i < (m_ar/bkSize); i++)
        for(k = 0; k < (m_br/bkSize); k++)
            for(j = 0; j < (m_ar/bkSize); j++)
                for(int l = 0; l < bkSize; l++)
                    for(int m = 0; m < bkSize; m++)
                        for(int n = 0; n < bkSize; n++)
                            phc[l*m_ar + n + i*bkSize*m_ar + j*bkSize] += pha[l*m_ar + m + i*bkSize*m_ar +  k*bkSize] * phb[m*m_ar + n + k*bkSize*m_ar +  j*bkSize];
	
	Time2 = clock();
	sprintf(st, "Time: %3.3f seconds\n", (double)(Time2 - Time1) / CLOCKS_PER_SEC);
	cout << st;
	double gflops = (2*(pow(m_ar,3)))/((double)(Time2 - Time1) / CLOCKS_PER_SEC);
...
/*     ret = PAPI_add_event(EventSet,PAPI_TLB_DM);
    if (ret != PAPI_OK) cout << "ERROR: PAPI_TLB_DM" << endl;
